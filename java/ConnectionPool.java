 ...
 * Connection Pool
 *
 * Contains pools for read, write and bulk requests. Each pool is
 * primed to a minimum size based on its configured properties.
 * A pool can grow beyond a minimum size to a configured maximum.
 * Minimal eviction logic exists wherein connections are evicted from
 * the available pool if the available pool has grown beyond the
 * minimum size for a configured interval. The configured interval
 * is reached after successive timer pops of the eviction interval.
 * For instance, if for 3 successive timer pops the available pool
 * has remained beyond the minimum size then we can 'assume' that the
 * pool is in a 'quiescent' state since it hasn't been flooded with
 * a barrage of new requests. Connections are ordered based on a last
 * updated timestamp in ascending order. Connections which have been
 * idle for the longest period are at the front of the queue and get
 * evicted first.
 */
